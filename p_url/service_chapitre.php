...
class f_af extends url{
    ...
    public $f_af_image = '<div class=" shadow-sm-sm bg-dark   p-3 mb-3  rounded"><div class="row no-gutters bg-light position-relative">
...
public function f_contenu_formation($___db,$__array_db,$__exploide,$__session_formation,$__token){ 
	...
	if($_count>=3):	
	$id_formation = $d_exploide[0];// Selection formation 
	$id_module= $d_exploide[1];// valeur de ID
	$type_formation= $d_exploide[2];// valeur de ID

$sql='SELECT 
'.$__array_db['formation'].'.listeformation.formation as formarion,
...
WHERE 
'.$__array_db['formation'].'.moduleform.idmodule=? 
AND
...
$connect_db->execute(array($id_module));
